...
def createMongoConnection(config_params, siteId, mode, logger):
    ...
    try:
        ...
        try:
            ...
            logger.critical('connection.createMongoConnection - Site ID {siteId} - Datastore {datastore} or Collection {collection} does not exist, CREATING'.format(
                siteId=siteId,
...
def create_mongo_ts_collection(db, config_params, siteId, logger):
    ...
    try:
        ...
        logger.debug('connection.create_ts_collection - Site ID {siteId}, Collection {collection}, CREATED'.format(
            siteId=siteId,
            collection=config_params["MONGO_COLLECTION"]
        ))
        return 0
        ...
        logger.critical('connection.create_mongo_ts_collection - Site ID {siteId}, Collection {collection}, Create FAILED, Err: {err}'.format(
            siteId=siteId,
            collection=config_params["MONGO_COLLECTION"],
            err=e
        ))
