...
#   File                :   main.py
#
#   Description         :   In app4c we're going to add the code to enable writing our payload to a File per site configured.
#                       :   Each line in the file will be a JSON document as a flattened representation of the payload.
#                       :   Before we add the  write to file code however you will notice I moved the 2 MongoDB functions,
#                       :   where by I create a connnect and then insertMongo behind 2 virtual calls.
#                       :   This allows us to use one call for opening a connection from our code and one call for the saving/writing to
#                       :   to the data store. In the generic function we determine the type of data store we are using and then execute the correct
#                       :   function.
#
...
def main():
    ...
    try:
        for site in my_sites:
            ...
            logger.info("Calling simulate.run_simulation for {siteId}".format(
                siteId=site["siteId"],
            ))
