...
#   File            :   simulate.py
#
#   Description     :   In app4d we're improving our interfacing with MongoDB a bit, we're going to check if the database exist,
#                   :   if not we create it. Then we'll check if the collection exists, if not we create it.
#
...
def run_simulation(site, current_time, config_params):
    ...
    logger.debug("Running in process -------> {process_name}".format(
